#https://jokerpython33.pythonanywhere.com/?id=1 // data
#راح اشرح شون ترتبون اتصالات وتسوها اداة
#واشرح فكرة متاحات
#متاح فكرته يعني يكون ايميل مستخدم في انستكرام او من اي موقع كان وبعدها تفحص ايميل في جيميل او هوتميل او غيرها من دومينات مشهورة  اذا طلع غير مستخدم يعني هذا متاح هاي فكرة متاحات
#راح جمع اتصالات بي class لازم يكون عندك معرفة عن برمجة كاىنية توجه او شىيه OOP
import requests,random,threading
#threading => علمود سرعة
class Instagram:
    def __init__(self):
        #هاي دالة __init__ ماترجع قيمة 
        #يعني ماتكدر تسويلها return
        self.hit=0
        self.bad_insta =0
        self.bad_gmail=0
        for i in range(5):#اذا تريدون تسرعوها زيدو 5
    ...
    def get_info(self,username):
        ...
        try:
            ...
            #هذا api يجبلكم تاريخ انشاء حساب انسته من ايدي
            #هيج تستدعي دالة بي OOP 
            #اذا حطيت بي قوسين دالة self
            #self.اسم دالة
...
email : {username}@gmail.com
        ...
        correct : {cor}
            ...
            with open("hit.txt","a",encoding="utf-8") as f:
                f.write(message+"\n")
            #تكدرون تحطون ارسال لي تيلكرام بي telebot او تحفضونه بي ملف بي with open
        #هاي دالة تفحص معلومات متاح صح لو خطا تحتاج ايميل وريست حتى تتاكد تاخذ اول حرفين من ريست وايميل واخر حرفين واذا نفس حروف يعني معلومات صح
    def check_gmail(self,email):
        ...
        if "password" in response.text:
            print(True)
            self.get_info(email)
            #اذا ايميل غير مستخدم بي جيميل شغل دالة info
        else:
            print(False)
    def check_instagram(self,email):
        #هاي دالة فحص ايميل بل نسته
        url = "https://i.instagram.com/api/v1/accounts/send_recovery_flow_email/"
    ...
    def generate_username(self):
        while True:
            ...
            try:
                ...
                print(user)
                self.check_instagram(user+"@gmail.com")
                #يشغل دالة فحص ايميل بل انستا 
            except:
                pass
Instagram()
#مبروك اداة متاحات انسته يمكن اسوي شرح برمجة اداة متاحات تيك
#ترتيب عليكم نه ماعرف ارتب
#اداة راح انشرها بل كيت هاب مالتي
#by => s1 / نترو لمام نترو / نترو لتصليح سبالات زطبخ باجة